class ServicesController < ApplicationController
  ...
  def create
    ...
    if logged_in?
      if current_service
        ...
        current_user.services.create!(provider: omnihash[:provider],
                                      uid: omnihash[:uid])
      ...
      if current_service
        ...
        user         = User.new
        # not sure if all users will have names associated with their accounts
        user.name = omnihash[:info][:name]
        user.name = omnihash[:info][:name] if omnihash[:provider].to_s ==
        'developer'
        user.email = omnihash[:info][:email]
        user_service = user.services.build(provider: omnihash[:provider],
                                           uid: omnihash[:uid])
        if user.save!
  ...
  def omniauth_providers
    (OmniAuth::Strategies.local_constants.map(&:downcase) -
    %i(developer oauth oauth2)).map(&:to_s)
  end
